...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _setup_prompt_encoder(self):
        ...
        for name, module in self.base_model.model.named_children(): #minigptv2
        #for name, module in self.named_children(): #minigpt4
...
class LlamaTuningForCausalLM(PeftModel):
    """
    Peft model for Causal LM
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 添加虚拟token的个数
        current_num_virtual_tokens = virtual_tokens_num
        # softprompt位置
        lens = insert_num 

        prompts = self.get_prompt(batch_size=batch_size)  # b, n, d
        prompts = prompts.to(inputs_embeds.dtype)
        new_inputs_embeds = []
        new_attention_mask = []
        new_inputs_labels=[]
        ...
        for i in range(batch_size):
            ...
            if labels is not None:
                ...
                stem_labels = torch.full((current_num_virtual_tokens,), -100).to(labels.device)
                inputs_labels_left = labels[i, :lens]  # n, d
                inputs_labels_right = labels[i, lens:]
                new_inputs_labels.append(torch.cat([inputs_labels_left, stem_labels, inputs_labels_right], dim=0))
        ...
        labels = torch.stack(new_inputs_labels, dim=0)
