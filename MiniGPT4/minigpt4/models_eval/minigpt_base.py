...
from minigpt4.models_eval.base_model import BaseModel
from minigpt4.models_eval.modeling_llama import LlamaForCausalLM
...
class MiniGPTBase(BaseModel):
    ...
    def __init__(
        ...
        super().__init__(
            virtual_tokens_num=virtual_tokens_num,
            init_mode=init_mode,
            prompt_id=prompt_id,
        )
        ...
        self.prompt_list = []
        self.virtual_tokens_num = virtual_tokens_num
    ...
    def get_context_emb_det_zero(self, prompt, img_list, object_list):
        device = img_list[0].device
        prompt = prompt.replace('<ObjHere>', object_list)
        prompt_segs = prompt.split('<ImageHere>')
        assert len(prompt_segs) == len(img_list) + 1, "Unmatched numbers of image placeholders and images."
        seg_tokens = [
            self.llama_tokenizer(
    ...
    def get_context_emb_det(self, prompt, img_list, object_list,virtual_num):
        device = img_list[0].device
        prompt = prompt.replace('<ObjHere>', object_list)
        added_tokens = [f"[ref{i+1}]" for i in range(virtual_num)]
        added_tokens = "".join(added_tokens)
        prompt = prompt.replace('<Soft>',added_tokens)
        prompt_segs = prompt.split('<ImageHere>')
        seg_tokens = [
            self.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=i==0).to(device).input_ids # only add bos to the first seg
    ...
    def prompt_wrap_object_zero(self, img_embeds, atts_img, prompts, objs, lengths=None):
        if prompts is None or len(prompts) == 0:
            # prompts is not provided, just return the original image embedding
            return img_embeds, atts_img
        elif img_embeds is None:
            ...
            for idx, (each_img_embed, each_prompt, obj) in enumerate(zip(img_embeds, prompts, objs)):
                pn = each_img_embed.shape[-2]
                if lengths is not None:
                    each_img_embed = each_img_embed.reshape(-1, each_img_embed.shape[-1])
                    each_img_embed = each_img_embed[:lengths[idx] * pn]
                ...
                each_prompt = each_prompt.replace('<ObjHere>', obj)
                p_segs = each_prompt.split('<ImageHere>')
                interleave_emb = []
                for idx, seg in enumerate(p_segs[:-1]):
                    p_tokens = self.llama_tokenizer(
    ...
    def prompt_wrap_object(self, img_embeds, atts_img, prompts, objs, lengths=None):
        if prompts is None or len(prompts) == 0:
            # prompts is not provided, just return the original image embedding
            return img_embeds, atts_img
        elif img_embeds is None:
            ...
            for idx, (each_img_embed, each_prompt, obj) in enumerate(zip(img_embeds, prompts, objs)):
                ...
                p_segs = each_prompt.split('<ImageHere>')
                interleave_emb = []
                for idx, seg in enumerate(p_segs[:-2]):
                    p_tokens = self.llama_tokenizer(
                        seg, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
                    p_embed = self.embed_tokens(p_tokens.input_ids)
                    interleave_emb.append(torch.cat([p_embed, each_img_embed[None][:, idx * pn:(idx + 1) * pn]], dim=1)) #262
                wrapped_emb = torch.cat(interleave_emb, dim=1) 
                p_tokens = self.llama_tokenizer(
                    p_segs[-2], return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
                p_embed = self.embed_tokens(p_tokens.input_ids)
                wrapped_emb = torch.cat([wrapped_emb, p_embed], dim=1)

                #virtual token position
                self.insert_num = wrapped_emb.size(1)
    ...
    def preparing_embedding(self, samples):
        ...
        if 'conv_q' in samples:
            ...
            if 'length' in samples:
                ...
                #cond_embeds, cond_atts = self.prompt_wrap(img_embeds, img_atts, instruction) #origin model
                #cond_embeds, cond_atts = self.prompt_wrap_object_zero(img_embeds, img_atts, instruction, samples["object"]) #zero_object model
                cond_embeds, cond_atts = self.prompt_wrap_object(img_embeds, img_atts, instruction, samples["object"]) #virtual token model
    ...
    def forward(self, samples, reduction='mean'):
        ...
        inputs_embeds = torch.cat([bos_embeds, inputs_embeds], dim=1) #378
        attention_mask = torch.cat([bos_atts, attention_mask], dim=1) #378
        ...
        with self.maybe_autocast():
            outputs = self.llama_model(
                ...
                reduction=reduction,
                input_ids=None,
                insert_num=self.insert_num + 1, # add virtual tokens
                virtual_tokens_num=self.virtual_tokens_num, # default: 20
            )
        loss = outputs.loss
    ...
    def embed_tokens(self, token_ids):
        if hasattr(self.llama_model.base_model, 'model'): ## lora wrapped model
            embeds = self.llama_model.base_model.model.model.embed_tokens(token_ids) #minigptv2
            #embeds = self.llama_model.base_model.model.embed_tokens(token_ids) #minigpt4
        else:
    ...
    def generate_det(
        ...
        batch_embs = [self.get_context_emb_det_zero(text, img_list, objects) for text, img_list, objects in zip(texts, image_lists, objectss)]
    ...
    def load_tokens(self, model: LlamaForCausalLM, tokenizer, token_embedding_path=""):
        ...
        new_tokens_weights = new_tokens_weights.to(device)
        ...
        embedding_layer.weight.data = new_embedding_weights.to(device)
        model.set_input_embeddings(embedding_layer)
        return model, tokenizer,new_tokens_length
    
    def generate_det_softprompt(
        self,
        soft_path,
        images,
        texts,
        ...
        image_lists = [[image_emb[None]] for image_emb in img_embeds]
        obj_lists = [obj for obj in objects]

        self.llama_model,self.llama_tokenizer,new_tokens_length = self.load_tokens(self.llama_model,self.llama_tokenizer,soft_path)
        
        batch_embs = [self.get_context_emb_det(text, img_list, obj_list, new_tokens_length) for text, img_list, obj_list in zip(texts, image_lists, obj_lists)]
